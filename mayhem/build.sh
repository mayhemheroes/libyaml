#!/usr/bin/env bash
# libyaml/mayhem/build.sh — autotools build (ASan+UBSan) of libyaml + 10 libFuzzer harnesses
...
# Idempotency: if a prior build left autotools artifacts (Makefile + any .so), wipe them so
# the in-tree configure/make cycle always starts from a clean slate.  `make distclean` is a
# no-op when there's nothing to clean; tolerate a missing Makefile gracefully.
...
for harness in "$SRC"/mayhem/*_fuzzer.c; do
  ...
  $CC $SANITIZER_FLAGS $DEBUG_FLAGS -I"$SRC/include" -I"$SRC/mayhem" \
